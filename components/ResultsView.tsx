  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in-up w-full">
      <div className="text-center mb-10">
        <h2 className="text-4xl font-display font-bold mb-2">{t.complete}</h2>
        <p className="text-slate-400">{t.performance}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {/* Score Card */}
        <div className="col-span-1 bg-slate-800 border border-slate-700 rounded-2xl p-6 flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <h3 className="text-slate-300 font-medium">{t.accuracy}</h3>
        </div>
        ...
        <div className="col-span-1 md:col-span-2 grid grid-cols-2 gap-4">
           <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 flex flex-col justify-between">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-slate-400 text-sm mb-1">{t.totalQ}</p>
           ...
           <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 flex flex-col justify-between">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-slate-400 text-sm mb-1">{t.correct}</p>
           ...
           <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 flex flex-col justify-between col-span-2">
              <div className="flex items-center gap-4">
                 <div className="p-3 bg-amber-500/10 rounded-xl text-amber-400">
                    <Award className="w-6 h-6" />
                 </div>
                 <div>
                    <p className="text-slate-400 text-sm">{t.verdict}</p>
      ...
      <div className="space-y-4 mb-10">
        <h3 className="text-xl font-bold text-white mb-4">{t.review}</h3>
            ...
            <div key={q.id} className={`p-4 rounded-xl border ${isCorrect ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-red-500/30 bg-red-500/5'}`}>
              <div className="flex gap-3">
                ...
                <div className="flex-1">
                  <p className="text-slate-200 font-medium mb-2">{q.text}</p>
                  <div className="text-sm text-slate-400 mb-2">
                    <span className="mr-2">{t.yourAns} <span className={isCorrect ? 'text-emerald-400' : 'text-red-400'}>{q.options[userAns] || 'Skipped'}</span></span>
                    {!isCorrect && <span className="text-emerald-400"> | {t.correctIs} {q.options[q.correctAnswerIndex]}</span>}
                  </div>
      ...
      <div className="flex gap-4 justify-center">
        <button 
          onClick={onHome}
          className="px-6 py-3 rounded-xl font-semibold bg-slate-700 text-white hover:bg-slate-600 transition-all flex items-center gap-2"
        >
