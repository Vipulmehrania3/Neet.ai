...
const App = () => {
  ...
  const renderHome = () => (
    <div className="flex flex-col items-center justify-center min-h-[80vh] w-full px-4 animate-fade-in">
      <div className="text-center mb-12">
        <h1 className="text-5xl md:text-7xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-r from-brand-300 via-brand-100 to-white mb-4">
          NEET.ai
        </h1>
        ...
        {SUBJECTS.map((subject) => {
          ...
          return (
            <button
              ...
              className="group relative bg-slate-800/50 hover:bg-slate-800 border border-slate-700 hover:border-brand-500/50 rounded-2xl p-8 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-brand-500/10 text-left overflow-hidden"
            >
              ...
              <h3 className="text-2xl font-bold text-slate-100 mb-1">{subject.name}</h3>
              <p className="text-sm text-slate-400">{subject.chapters.length} Chapters • High Yield</p>
    ...
    return (
      <div className="w-full max-w-4xl mx-auto px-4 h-full flex flex-col">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h2 className="text-3xl font-display font-bold text-white">{t.selectChapters}</h2>
            <p className="text-slate-400">{t.from} {selectedSubject.name}</p>
          </div>
        </div>
             ...
             return (
              <div 
                ...
                className={`group flex items-center justify-between p-4 rounded-xl border transition-all duration-200 cursor-pointer
                  ${isSelected ? 'bg-brand-900/20 border-brand-500/50' : 'bg-slate-800/30 border-slate-700 hover:border-slate-500'}
                `}
                onClick={() => toggleChapter(chapter.id)}
              >
                <div className="flex items-center gap-4">
                  <div className={`w-6 h-6 rounded-md border-2 flex items-center justify-center transition-colors
                    ${isSelected ? 'bg-brand-500 border-brand-500' : 'border-slate-600 group-hover:border-slate-400'}
                  `}>
                    {isSelected && <Sparkles className="w-3 h-3 text-white" />}
                  </div>
                  <div>
                    <h4 className={`font-medium ${isSelected ? 'text-white' : 'text-slate-300'}`}>{chapter.name}</h4>
                    {selectedTopicCount > 0 && (
                      <span className="text-xs text-brand-400">{selectedTopicCount} topics selected</span>
                    )}
                  </div>
                </div>
                ...
                <button
                  ...
                  className="p-2 text-xs font-medium text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg border border-slate-700 transition-colors z-10"
                >
                  {t.selectTopics}
                </button>
              </div>
        ...
        {activeTopicSidebar && (
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            />
            <div className="relative w-full max-w-md bg-slate-900 h-full border-l border-slate-700 shadow-2xl p-6 overflow-y-auto animate-slide-in-right">
              ...
              <div className="space-y-4">
                <p className="text-sm text-slate-400 uppercase tracking-wider font-semibold">Select Specific Topics</p>
                  ...
                  ?.topics.map(topic => {
                    ...
                    return (
                      ...
                      >
                        <input 
                          type="checkbox"
                          className="w-5 h-5 rounded border-slate-600 text-brand-500 focus:ring-brand-500 bg-slate-700"
                        ...
                        />
                        <span className={isTopicSelected ? 'text-white' : 'text-slate-300'}>{topic.name}</span>
                      </label>
                    );
                  })
                }
  ...
  const renderQuizSetup = () => (
    <div className="w-full max-w-2xl mx-auto px-4 animate-fade-in-up">
      <div className="mb-8">
        ...
        <h2 className="text-3xl font-display font-bold text-white mb-2">{t.configure}</h2>
        <p className="text-slate-400">{t.customize}</p>
      </div>

      <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 md:p-8 space-y-8 backdrop-blur-sm">
        
        {/* Question Count */}
        <div>
          <div className="flex justify-between mb-4">
            <label className="text-slate-200 font-medium flex items-center gap-2">
              <Settings2 className="w-4 h-4 text-brand-400" /> {t.numQuestions}
            </label>
            <span className="text-brand-400 font-bold text-xl">{quizConfig.questionCount}</span>
          </div>
          <input 
            type="range" 
            min="5" 
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full bg-slate-900 border border-slate-600 rounded-xl p-4 text-slate-200 placeholder-slate-500 focus:ring-2 focus:ring-brand-500 outline-none h-32 resize-none"
          />
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-50 font-sans selection:bg-brand-500/30 selection:text-brand-200 overflow-x-hidden">
      ...
      <nav className="relative z-10 w-full p-6 flex justify-between items-center border-b border-white/5 bg-slate-900/50 backdrop-blur-md sticky top-0">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setView(AppView.HOME)}>
           ...
           <span className="font-display font-bold text-xl tracking-tight text-white">NEET.ai</span>
        </div>
        
        {/* Language Toggle */}
        <button 
          onClick={toggleLanguage}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-700 bg-slate-800/50 hover:bg-slate-700 transition-colors text-slate-300"
        >
